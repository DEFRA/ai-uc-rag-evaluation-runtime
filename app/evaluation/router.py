...
async def trigger_rag_evaluation(
    ...
    """
    try:
        content = await rag_service.query_snapshot(
            group_id, query, max_results
        )
...
async def testing() -> JSONResponse:
    ...
    result = await evaluation_service.evaluate_pydantic(knowledge, question, answer, evaluation_service.settings)
